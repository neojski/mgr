...
\begin{lem} \label{lem:basic-atlas}
  Niech $Y$ będzie przestrzenią liniowo-metryczną, $M$ przestrzenią topologiczną. Wówczas następujące warunki są równoważne:
  \begin{enumerate}
  ...
  \begin{proof}
    ...
    Skoro $\{B(f(x), \varepsilon)\}_{0 < \varepsilon < \varepsilon_0}$ jest bazą w punkcie $f(x)$, to $\{G_\varepsilon\}_{0 < \varepsilon < \varepsilon_0}$ jest bazą w punkcie $x$.
  \end{proof}
\end{lem}

\begin{lem}
  Niech $Y$ będzie przestrzenią liniowo-metryczną, a $M$ spójną rozmaitością topologiczną modelowaną na $Y$. Wówczas:
  \[
    \wght Y = \wght M
  \]
  \begin{proof}
    Jeśli $Y=\{0\}$, to twierdzenie jest prawdziwe. Niech więc $\wght Y\geq\aleph_0$.
...
\begin{lem} \label{lem:atlas-countable}
  ...
  \begin{proof}
    ...
    Zatem możemy pewien podzbiór rodziny $\mathcal B$ ponumerować elementami zbioru $C_n$, mianowicie $\mathcal B \supset \mathcal B_n := (B_{c,n})_{c \in C_n}$.
