...
\begin{fact}
  ...
  Przestrzeń liniowo-metryczna z operacją $+$ stanowi grupę topologiczną z topologią zadaną przez metrykę, a więc spełniającą pierwszy aksjomat przeliczalności oraz własność $T_2$. Z twierdzenia Birkhoffa-Kakutaniego \cite{bir}, \cite{kak} istnieje więc na niej metryką lewostronnie niezmiennicza.
  \end{proof}
\end{fact}
...
\begin{lem} \label{lem:ball-homeomorphism}
  ...
  \begin{proof}
    ...
    \]
    Istotnie. Funkcja $g$ jest poprawnie określona, bo dla $z\in U_\varepsilon$ mamy $\lambda(y) < 1$. Pokażemy teraz, że $g(h(y)) = y$. Jeśli $\lambda(y) \leq \frac{1}{2}$, to $h(y) = y$ oraz $g(y) = y$. Zatem $g(h(y)) = y$. Jeśli natomiast $\lambda(y) > \frac{1}{2}$, to $h(y) = \frac{y}{\lambda(y)+\frac{1}{2}}$. Wówczas jednak $\lambda(h(y)) = \frac{\lambda(y)}{\lambda(y)+\frac{1}{2}} > \frac{1}{2}$, ponieważ dla $s \geq 0$:
    \begin{align*}
%      t > \frac{1}{2} \iff 2t > 1 \iff t > 1-t \iff \frac{t}{2(1-t)} > \frac{1}{2}
