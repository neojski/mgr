...
\begin{note}
  Jeśli $(Y, d)$ przestrzeń liniowo-metryczna z metryką niezmienniczą. Wówczas metryka $\rho := \min(d, 1)$ jest ograniczoną, niezmienniczą metryką równoważną.
  \begin{proof}
...
\begin{lem}
  Aby metryka $d$ była słabo rosnąca po promieniach wystarczy aby warunek $d(t_1 x, 0) < d(t_2 x, 0)$ zachodził dla liczb wymiernych $t_1, t_2 \geq 0$.
  
  \begin{proof}
    Niech $0 \leq a < b$ będą liczbami rzeczywistymi. Wówczas istnieją ciągi $(a_n)_{n=1}^\infty$ oraz $(b_n)_{n=1}^\infty$ liczb wymiernych  takie, że $a_n \downarrow a$ i $b_n \uparrow b$, takie że:
    \[\forall n \in \mathbb{N}_1: d(a_n x, 0) < d(b_n x, 0)\]
    
    Ale ze wzrostu po promieniach po liczbach wymiernych wnioskujemy:
    \[d((b_n - a_n) x, 0) \geq d((b_1 - a_1) x, 0) =: \varepsilon > 0\]
    
    A więc:
    \[d(a_n x, 0) + \varepsilon \leq d(b_n x, 0)\]
    
    Co po przejściu granicznym daje:
    \[d(ax, 0) + \varepsilon \leq d(bx, 0)\]
    
    W szczególności $d(ax, 0) < d(bx, 0)$. 
  \end{proof}
\end{lem}
...
\begin{thm}[Eidelheit-Mazur] \label{thm:eidelheit-mazur}
  ...
  \begin{proof}
    Określamy dla $w > 0$ metrykę $\rho_w(x, y) := \sup_{0 \leq t \leq w} d(tx, ty)$. $\rho_w$ jest metryką, jako skończone supremum metryk. Z niezmienniczości $d$ wynika niezmieniczość $\rho_w$. Co więcej, jest metryką równoważną $d$.
    ...
    Z drugiej strony, niech $\varepsilon > 0$. Zdefiniujmy odwzorowanie:
    \[m: [0,w] \times Y \ni (t,x) \to d(tx, 0) \in Y\]
    ...
    \end{align*}
    A więc
    \[d(t_2 x, 0) \leq \sup_{0 \leq t \leq 1} d(t \cdot t_2 x, 0) = \rho_1(t_2 x, 0) = \sup_{0 \leq s \leq t_1^i t_2^{-i}} d(tx, 0)\]
    Z rozważań nad $m$ wiemy jednak, że wraz z maleniem ciągu $t_1^i t_2^{-i}$ do zera $\sup_{0 \leq s \leq t_1^i t_2^{-i}} d(tx, 0)$ również dąży do zera. Zatem $d(t_2 x, 0) = 0$, co w konsekwencji daje $x = 0$. Zatem jedynym przypadkiem, gdy nie zachodzi wzrost po promieniach jest $x = 0$, zatem $\rho$ jest metryką rosnącą po promieniach.
